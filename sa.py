...
bounds = {
    ...
    "Griewank": [[-550, 500], [-550, 500], [-550, 500], [-550, 500], [-550, 500],
                 [-550, 500], [-550, 500], [-550, 500], [-550, 500], [-550, 500]],
    "PriceTransistor": [[0, 10], [0, 10], [0, 10], [0, 10], [0, 10], [0, 10],
                        [0, 10], [0, 10], [0, 10]],
    "Expo": [[-12, 10], [-12, 10], [-12, 10], [-12, 10], [-12, 10], [-12, 10],
             [-12, 10], [-12, 10], [-12, 10], [-12, 10]],
    "Modlangerman": [[0, 10], [0, 10], [0, 10], [0, 10], [0, 10], [0, 10],
                     [0, 10], [0, 10], [0, 10], [0, 10]],
    "EMichalewicz": [[0, np.pi], [0, np.pi], [0, np.pi], [0, np.pi], [0, np.pi]],
    "Shekelfox5": [[0, 10], [0, 10], [0, 10], [0, 10], [0, 10]],
    "Schwefel": [[-500, 500], [-500, 500], [-500, 500], [-500, 500], [-500, 500],
...
# objective function
def Objective(par, fn):
    if fn == "Schaffer1":
        return Schaffer1(par)
    elif fn == "Schaffer2":
        return Schaffer2(par)
    elif fn == "Salomon":
        return Salomon(par)
    elif fn == "Griewank":
        return Griewank(par)
    elif fn == "PriceTransistor":
        return PriceTransistor(par)
    elif fn == "Expo":
        return Expo(par)
    elif fn == "Modlangerman":
        return Modlangerman(par)
    elif fn == "EMichalewicz":
        return EMichalewicz(par)
    elif fn == "Shekelfox5":
        return Shekelfox5(par)
    else:
        return Schwefel(par)
...
def Bounds(fn):
    if fn == "Schaffer1":
        return bounds.get("Schaffer1")
    elif fn == "Schaffer2":
        return bounds.get("Schaffer2")
    elif fn == "Salomon":
        return bounds.get("Salomon")
    elif fn == "Griewank":
        return bounds.get("Griewank")
    elif fn == "PriceTransistor":
        return bounds.get("PriceTransistor")
    elif fn == "Expo":
        return bounds.get("Expo")
    elif fn == "Modlangerman":
        return bounds.get("Modlangerman")
    elif fn == "EMichalewicz":
        return bounds.get("EMichalewicz")
    elif fn == "Shekelfox5":
        return bounds.get("Shekelfox5")
    else:
        return bounds.get("Schwefel")


def GenSA(fn, control):
    # Get bounds
    bounds = Bounds(fn)

    # Generate initial solution
    best = [d[0] for d in bounds] + np.random.rand(len(bounds)) * (np.array([d[1] for d in bounds]) - np.array([d[0] for d in bounds]))
    # Evaluate the initial solution
    best_eval = Objective(best, fn)
    ...
    for i in range(n_iterations):
        # take a step
        candidate = curr + np.random.randn(len(bounds)) * 10
        # evaluate candidate point
        candidate_eval = Objective(candidate, fn)
...
def main(argv):
    ...
    for fn in fns:
        best = GenSA(fn, {})
        print(fn, ": opt: ", optimums.get(fn), " result: ", best, " diff: ", np.abs(optimums.get(fn)) - best)
        print('\n')
